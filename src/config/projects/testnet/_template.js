// Template for creating new TESTNET project configurations
...
const projectTemplate = {
  ...
  networkLabel: "Testnet",             // This should be "Testnet" for testnet projects
  logo: "/logo-yourproject.png",       // Logo file in public/ folder
  chainId: "your-chain-id-testnet",    // Actual testnet chain ID
...
sudo mv yourprojectd /usr/local/bin/`,
    ...
    initNodeCmd: (nodeName) => `yourprojectd init ${nodeName || "<YOUR_NODE_NAME>"} --chain-id your-chain-id-testnet
wget -O $HOME/.yourproject/config/genesis.json "https://raw.githubusercontent.com/yourproject/testnet/main/genesis.json"`,
    
    // Command to configure seeds and peers
    configNodeCmd: `SEEDS="seed1@testnet.yourproject.io:26656,seed2@testnet.yourproject.io:26656"
PEERS="peer1@testnet-ip1:26656,peer2@testnet-ip2:26656"
...
[Unit]
Description=Your Project Testnet Node
...
sudo systemctl restart yourprojectd`,
    ...
    autoInstallCmd: `wget -O yourproject-testnet.sh https://testnet.yourproject.com/yourproject.sh && chmod +x yourproject-testnet.sh && ./yourproject-testnet.sh`,
  },
  
  // Project URLs and links
  urls: {
    repo: "https://github.com/yourproject/testnet.git",
    genesisUrl: "https://raw.githubusercontent.com/yourproject/testnet/main/genesis.json",
    docsUrl: "https://docs.yourproject.io/testnet",
    explorerUrl: "https://testnet-explorer.yourproject.io",
    officialSite: "yourproject.io"
  },
  
  // RPC endpoints for the Public RPC Scanner page
  rpcEndpoints: [
    { url: 'https://testnet-rpc.yourproject.io/', status: 'active', height: '1,000,000' },
    { url: 'https://testnet-rpc2.yourproject.io/', status: 'active', height: '1,000,000' },
    { url: 'https://testnet-rpc3.yourproject.io/', status: 'inactive', height: 'N/A' },
  ],
  
  // API endpoints for the API & Sync page
  apiEndpoints: {
    rpc: `https://testnet-rpc.yourproject.io/`,
    api: `https://testnet-api.yourproject.io/`,
    grpc: `testnet-grpc.yourproject.io:9090`,
    stateSync: {
      enableCmd: `sed -i -e "s|^enable *=.*|enable = true|" $HOME/.yourproject/config/config.toml`,
      rpcServersCmd: `RPC_SERVERS="https://testnet-rpc.yourproject.io:443,https://testnet-rpc.yourproject.io:443"
sed -i -e "s|^rpc_servers *=.*|rpc_servers = \\"$RPC_SERVERS\\"|" $HOME/.yourproject/config/config.toml`,
      trustHeightCmd: `LATEST_HEIGHT=$(curl -s https://testnet-rpc.yourproject.io/block | jq -r .result.block.header.height)
TRUST_HEIGHT=$((LATEST_HEIGHT - 1000))
TRUST_HASH=$(curl -s "https://testnet-rpc.yourproject.io/block?height=$TRUST_HEIGHT" | jq -r .result.block_id.hash)
